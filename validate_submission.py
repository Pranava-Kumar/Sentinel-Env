...
def check_environment_variables():
    ...
    check('os.getenv("API_BASE_URL"' in content or "os.getenv('API_BASE_URL'" in content,
          "API_BASE_URL has default value")
    check('os.getenv("MODEL_NAME"' in content or "os.getenv('MODEL_NAME'" in content,
...
def check_openai_client():
    ...
    check("from openai import OpenAI" in content, "OpenAI imported from openai package")
    check("OpenAI(" in content, "OpenAI client instantiated")
    check("client.chat.completions.create" in content or ".chat.completions.create" in content,
...
def check_structured_logging():
    ...
    check("task=" in content and "env=" in content and "model=" in content,
          "[START] includes task, env, model fields")
    check("step=" in content and "action=" in content and "reward=" in content,
          "[STEP] includes step, action, reward fields")
    check("success=" in content and "steps=" in content and "score=" in content,
...
async def check_hf_space_health():
    ...
    space_url = "https://PranavaKumar09-sentinel-env.hf.space"
...
async def check_endpoints():
    ...
    try:
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            check(reset_resp.status_code == 200, f"POST /reset returns 200")
            ...
            if reset_resp.status_code == 200:
                ...
                check(step_resp.status_code == 200, f"POST /step returns 200")
                ...
                check(state_resp.status_code == 200, f"GET /state returns 200")
                
                # Test /grade
                grade_resp = await client.get(f"{space_url}/grade", headers={"X-Episode-ID": episode_id})
                check(grade_resp.status_code == 200, f"GET /grade returns 200")
